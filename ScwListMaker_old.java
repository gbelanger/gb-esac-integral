...
public final class ScwListMaker {
	...
	for ( int i=0; i < nscw; i++ ) {
	    ...
	    // Only take pointings with duration > 900 seconds
	    if ( telapse[i] > 900 ) {
		// First check is the revNum is within range
		if ( revNum <= lastRev && revNum >= firstRev ) {
		    ...
		    // Second check if the distance is within the maximum diagonal distance from pointing
		    if ( distInDeg <= maxDiagDist && distInPix < maxDiagDistInPix ) {
			// Third check if the latitude is within range
			double pointing_b = pointing_lb.getY();
			if ( pointing_b >= bMin && pointing_b < bMax ) {
			    // Finally check if the longitude is within range
			    double pointing_l = pointing_lb.getX();
			    // Allow for negative values and account for wrap around at l=0 
			    if ( lMin > lMax ) {
	...
	String[] ids = (String[]) selectedIDsList.toArray();
